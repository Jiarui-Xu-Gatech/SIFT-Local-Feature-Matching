...
def get_gradient_histogram_vec_from_patch(
    ...
    N=128
    pi=np.pi
    angle=np.array([-7*pi/8,-5*pi/8,-3*pi/8,-1*pi/8,1*pi/8,3*pi/8,5*pi/8,7*pi/8])
    wgh=np.zeros(N)
    ...
    for j in range(4):
        for w in range(4):
            for p in range(4):
                for q in range(4):
                    for i in range(8):
                        if window_orientations[j*4+p,w*4+q]+1e-6<angle[i]+pi/8 and window_orientations[j*4+p,w*4+q]+1e-6>=angle[i]-pi/8:
                            wgh[j*4*8+w*8+i]+=window_magnitudes[j*4+p,w*4+q]
    wgh=wgh.reshape(128,1)
    '''
    for grid_i in range(4):
        for grid_j in range(4):
            grid_ori = window_orientations[grid_i * 4: grid_i * 4 + 4, grid_j * 4: grid_j * 4 + 4]
            grid_mag = window_magnitudes[grid_i * 4: grid_i * 4 + 4, grid_j * 4: grid_j * 4 + 4]

            grid_his = np.zeros([1, 8])
            for i in range(4):
                for j in range(4):
                    ori = int(round((grid_ori[i, j] + np.pi) / (np.pi / 4), 3))
                    if ori == 8:
                        grid_his[0, 7] = grid_his[0, 7]+grid_mag[i, j]
                    else:
                        grid_his[0, ori] = grid_his[0, ori]+grid_mag[i, j]
            if grid_i == 0 and grid_j == 0:
                wgh = grid_his
            else:
                wgh = np.concatenate((wgh, grid_his), axis=0)
    wgh = wgh.reshape(128, 1)
    '''
...
def get_feat_vec(
    ...
    m=magnitudes[int(r)-int(feature_width/2)+1:int(r)+int(feature_width/2)+1,int(c)-int(feature_width/2)+1:int(c)+int(feature_width/2)+1]
    o=orientations[int(r)-int(feature_width/2)+1:int(r)+int(feature_width/2)+1,int(c)-int(feature_width/2)+1:int(c)+int(feature_width/2)+1]
    fv=get_gradient_histogram_vec_from_patch(m,o)
...
def get_SIFT_descriptors(
    ...
    m,o=get_magnitudes_and_orientations(Ix,Iy)
    for i in range(X.shape[0]):
        fvs[i]=np.squeeze(get_feat_vec(X[i],Y[i],m,o,feature_width))
